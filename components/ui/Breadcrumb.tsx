  ...
  return (
    <div className="border-b border-line bg-cream">
      <Container className="flex flex-wrap items-center gap-2 py-4 text-xs font-semibold tracking-[0.1em] uppercase">
        <Link href="/" prefetch className="text-mist transition-colors hover:text-gold-deep">
          Home
        </Link>
        {items.map((item, i) => (
          <span key={`${item.label}-${i}`} className="flex items-center gap-2">
            ...
            {item.href ? (
              <Link href={item.href} prefetch className="text-mist transition-colors hover:text-gold-deep">
                {item.label}
              </Link>
            ) : (
